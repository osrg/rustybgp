...
mod tests {
    ...
    // GoBGP-generated test vectors (packet/tests/fixtures/gen/vpn_nlri/main.go).
    ...
    fn assert_v4_roundtrip(bytes: &[u8]) -> VpnV4Nlri {
        let mut c = Cursor::new(bytes);
        let nlri = VpnV4Nlri::decode(&mut c, bytes.len()).expect("decode failed");
        let mut buf = Vec::new();
        nlri.encode(&mut buf);
        assert_eq!(buf, bytes);
        nlri
    ...
    fn assert_v6_roundtrip(bytes: &[u8]) -> VpnV6Nlri {
        let mut c = Cursor::new(bytes);
        let nlri = VpnV6Nlri::decode(&mut c, bytes.len()).expect("decode failed");
        let mut buf = Vec::new();
        nlri.encode(&mut buf);
        assert_eq!(buf, bytes);
        nlri
    ...
    #[test]
    fn gobgp_vpnv4_single_label() {
        let nlri = assert_v4_roundtrip(GOBGP_VPNV4_SINGLE_LABEL);
    ...
    #[test]
    fn gobgp_vpnv4_label_stack() {
        let nlri = assert_v4_roundtrip(GOBGP_VPNV4_LABEL_STACK);
    ...
    #[test]
    fn gobgp_vpnv4_default() {
        let nlri = assert_v4_roundtrip(GOBGP_VPNV4_DEFAULT);
        assert_eq!(nlri.labels.labels().len(), 1);
    ...
    #[test]
    fn gobgp_vpnv6_single_label() {
        let nlri = assert_v6_roundtrip(GOBGP_VPNV6_SINGLE_LABEL);
        assert_eq!(nlri.labels.labels().len(), 1);
    ...
    #[test]
    fn gobgp_vpnv6_label_stack() {
        let nlri = assert_v6_roundtrip(GOBGP_VPNV6_LABEL_STACK);
        assert_eq!(nlri.labels.labels().len(), 2);
        assert_eq!(nlri.labels.labels()[0].value(), 100);
        assert_eq!(nlri.labels.labels()[1].value(), 200);
        assert_eq!(nlri.rd, rd());
